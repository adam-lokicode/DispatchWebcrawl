...
// NUCLEAR OPTION: Zero contact data processing
const CONFIG = {
    intervalMinutes: 5,    
    maxEntries: 50,        
    outputFile: 'dat_one_loads_no_contact.csv',
    statsFile: 'scraper_stats_no_contact.json',
    runImmediately: true,  
    enablePagination: true,
    extractContacts: false // DISABLED - Never extract contacts
};
...
async function runNuclearScraping() {
    const timestamp = new Date().toISOString();
    console.log(`\n🚀 Starting NUCLEAR CONTACT-FREE scraping run at ${timestamp}`);
    console.log(`📊 Target: ${CONFIG.maxEntries} entries${CONFIG.enablePagination ? ' (with pagination)' : ''}`);
    console.log('☢️  NUCLEAR MODE: ZERO contact data extraction - NO Thunderbird/FaceTime triggers possible');
    
    // Kill apps proactively
    try {
        const { exec } = require('child_process');
        exec('pkill -f thunderbird 2>/dev/null || true', () => {});
        exec('pkill -f FaceTime 2>/dev/null || true', () => {});
        console.log('🛡️  Proactively killed Thunderbird/FaceTime');
    } catch (error) {
        // Ignore errors
    }
    ...
    try {
        ...
        console.log(`📋 Processing ${loadRows.length} of ${allLoadRows.length} total loads (CONTACT-FREE mode)`);
        ...
        for (let idx = 0; idx < loadRows.length; idx++) {
            try {
                ...
                // CONTACT-FREE extraction - NO contact data at all
                const basicInfo = await row.evaluate(el => {
                    ...
                    if (companyCell) {
                        ...
                        // ZERO contact extraction - completely skipped
                    }
                ...
                console.log(`☢️  Contact: [NUCLEAR_MODE_DISABLED]`);
                
                await row.click();
                await page.waitForTimeout(getRandomDelay(1500, 3000));
                
                // Reference ID extraction (still works without contact data)
                const detailedInfo = await page.evaluate(() => {
                    let referenceNumber = 'N/A';
                    
                    // Look for Reference ID in the modal
                ...
                const loadData = {
                    ...
                    contact: null, // ALWAYS NULL - no contact data ever
        ...
        console.log(`\n✅ Extracted ${extractedData.length} loads in NUCLEAR CONTACT-FREE mode`);
        ...
        if (extractedData.length > 0) {
                        ...
                        fs.createReadStream(csvPath)
                            ...
                            .on('data', (data) => {
                                results.push({
                                    ...
                                    contact: null, // Always null in nuclear mode
            ...
            if (newRecords.length > 0) {
                ...
                console.log(`💾 ${newRecords.length} new records saved to CSV (CONTACT-FREE)`);
                ...
                console.log(`☢️  ZERO contacts extracted (nuclear mode) | 🔢 ${refCount} references found`);
            } else {
        ...
        console.log(`✅ NUCLEAR CONTACT-FREE scraping completed at ${new Date().toISOString()}`);
...
function startNuclearScheduledScraping() {
    console.log(`☢️  Starting NUCLEAR CONTACT-FREE scheduled scraper:`);
    ...
    console.log(`   ☢️  NUCLEAR MODE: ZERO contact extraction - impossible to trigger apps`);
    ...
    if (CONFIG.runImmediately) {
        console.log('\n🚀 Running initial nuclear scrape...');
        runNuclearScraping();
    }
    ...
    console.log(`\n⏰ Nuclear scheduler started. Next run in ${CONFIG.intervalMinutes} minutes.`);
    console.log('Press Ctrl+C to stop.');
}

function stopNuclearScheduledScraping() {
    if (intervalId) {
        clearInterval(intervalId);
        intervalId = null;
        console.log('\n🛑 Nuclear scheduled scraping stopped.');
    }
}

// Handle graceful shutdown
process.on('SIGINT', () => {
    console.log('\n🛑 Stopping nuclear scheduler...');
    stopNuclearScheduledScraping();
    process.exit(0);
});
...
// Start the nuclear scheduler
if (require.main === module) {
    startNuclearScheduledScraping();
}
